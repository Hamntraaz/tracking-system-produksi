...
export default function AdminOrders({ data = {}, deviceLocation, requestLocation, refreshData, user }) {
  ...
  async function submitRequest(event) {
    ...
    try {
      ...
      await createPurchaseOrder({
        ...
        destination_address: selectedWarehouse?.address || user?.branch || selectedWarehouse?.name || user?.name || 'Gudang/Cabang Rafiza',
      })
  ...
  async function confirmReceive(order) {
    ...
    setMessage('')
    try {
      await receiveOrder({ order_id: order.id, created_by: user?.name || 'Admin Gudang/Cabang', warehouse_id: user?.warehouse_id || null })
  ...
  return (
    <>
      <section className="page-head-card"><div><span>Admin Gudang/Cabang</span><h2>Permintaan Barang ke Supplier</h2><p>Gudang memilih supplier. Bahan baku dan satuan otomatis mengikuti data supplier yang didaftarkan manajer.</p></div></section>
      ...
      <section className="content-grid two-one">
        ...
        <article className="panel-card">
          ...
          <form className="mini-form" onSubmit={submitRequest}>
            ...
            <p className="helper-box">Koordinat tujuan akan diambil dari izin lokasi browser admin/gudang, bukan titik toko statis.</p>
