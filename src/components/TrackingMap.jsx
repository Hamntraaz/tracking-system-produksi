...
function roleIcon(role = '') { const key = String(role || '').toLowerCase(); if (key === 'supplier') return '🏭'; if (key === 'warehouse') return '🏬'; if (key === 'branch') return '🏪'; if (key === 'courier') return '🏍️'; if (key === 'manager') return '📊'; return '📍' }
function roleTone(role = '', fallback = 'orange') { const key = String(role || '').toLowerCase(); if (key === 'supplier') return 'red'; if (key === 'warehouse') return 'green'; if (key === 'branch') return 'orange'; if (key === 'courier') return 'orange'; if (key === 'manager') return 'green'; return fallback }
function divMarker(label, tone, emoji = '') { return L.divIcon({ className: 'custom-map-marker', html: `<span class="pin ${tone}">${emoji}</span><b>${escapeHtml(label)}</b>`, iconSize: [150, 42], iconAnchor: [18, 32] }) }
function driverMarker(label = 'Kurir Live') { return L.divIcon({ className: 'driver-live-marker', html: `<span class="driver-pulse"></span><span class="driver-dot">🏍️</span><b>${escapeHtml(label)}</b>`, iconSize: [160, 52], iconAnchor: [26, 32] }) }
...
function MarkerPopup({ title, subtitle, address, status, updatedAt, points, delivery }) { return <div className="map-popup-card"><strong>{title}</strong>{subtitle && <span>{subtitle}</span>}{status && <p><b>Status</b><em>{status}</em></p>}{delivery?.code && <p><b>Kode</b><em>{delivery.code}</em></p>}{delivery?.order_code && <p><b>PO/REQ</b><em>{delivery.order_code}</em></p>}{address && <p><b>Alamat</b><em>{address}</em></p>}{updatedAt && <p><b>Update</b><em>{updatedAt}</em></p>}<button type="button" onClick={() => openExternalMaps(points)}>Lihat di Maps</button></div> }
...
export default function TrackingMap({
  ...
  hidePickup = false,
  useDeviceAsCurrent = false,
  useDeviceAsDestination = false,
  currentLabel,
}) {
  ...
  const destinationRole = useDeviceAsDestination ? viewerUser?.role : (delivery?.destination_role || 'warehouse')
  const pickupLabel = delivery?.pickup_label || delivery?.supplier_name || 'Supplier'
  const destinationLabel = useDeviceAsDestination ? viewerLocationLabel(viewerUser) : (delivery?.destination_label || delivery?.warehouse_name || 'Gudang')
  const courierLabel = currentLabel || (delivery?.courier_name ? `Kurir: ${delivery.courier_name}` : 'Kurir Live')
  ...
  const roadWaypoints = useMemo(() => {
    ...
    if (hidePickup) return [current, destination].filter(Boolean)
    return [pickup, current, destination].filter(Boolean)
  }, [delivery, pickup?.[0], pickup?.[1], current?.[0], current?.[1], destination?.[0], destination?.[1], hidePickup, showOnlyActive, trackingActive])

  const fitPoints = useMemo(() => {
    const base = [hidePickup ? null : pickup, current, destination].filter(Boolean)
    if (showUserLocation && userPoint && !useDeviceAsCurrent && !useDeviceAsDestination) return [...base, userPoint]
    return base
  }, [pickup?.[0], pickup?.[1], current?.[0], current?.[1], destination?.[0], destination?.[1], showUserLocation, userPoint?.[0], userPoint?.[1], hidePickup, useDeviceAsCurrent, useDeviceAsDestination])
  ...
  useEffect(() => {
    ...
    async function loadRoadRoute() {
      if (roadWaypoints.length < 2) { setRoadRoute([]); setRouteInfo(null); setRouteStatus('Rute belum bisa dibuat karena titik lokasi belum lengkap.'); return }
      ...
      try {
        ...
        if (!ignore && Array.isArray(bestRoute?.geometry) && bestRoute.geometry.length > 0) { setRoadRoute(bestRoute.geometry); setRouteInfo(bestRoute); setRouteStatus(`Rute aktif: ${formatDistance(bestRoute.distance_m)} • ${formatDuration(bestRoute.duration_s)}.`); return }
        ...
        try {
          ...
          if (!ignore && Array.isArray(coordinates) && coordinates.length > 0) { const routeGeometry = coordinates.map(([lng, lat]) => [lat, lng]); setRoadRoute(routeGeometry); setRouteInfo({ distance_m: best.distance, duration_s: best.duration, source: 'OSRM Public' }); setRouteStatus(`Rute OSRM aktif: ${formatDistance(best.distance)} • ${formatDuration(best.duration)}.`); return }
          throw new Error('Rute publik kosong')
        } catch {
          if (!ignore) { setRoadRoute(roadWaypoints); setRouteInfo(null); setRouteStatus('Rute jalan gratis tidak tersedia, sementara memakai garis titik-ke-titik.') }
        }
      }
    }
    loadRoadRoute()
  ...
  const emptyText = mapType === 'branch_request' ? 'Belum ada titik kurir/cabang untuk ditampilkan.' : 'Belum ada data pengiriman/lokasi untuk ditampilkan.'
